...
<body>
	...
	<div class="main_container">
		...
		<div class="right_container_full">
			<div class="page_title_div"><h1>VIEW JOB SHEET ENTRIES for <?php echo $_SESSION['session_store_name'];?></h1></div>
			...
			<div class="content_holder">
				...
				<table class="order_hisory_table">
										...
										if($result_key=='all' || $result_key==''){
											...
											//PAGINATION CODE
											$sql_pagin = "SELECT COUNT(entry_id) FROM `adm_cust_mob_add` WHERE  `store_id`=".$store_id." AND delete_status !=1 ORDER BY entry_id DESC";
											...
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND delete_status !=1 ORDER BY entry_id DESC LIMIT ".$start_from.", ".$offset."";							
											
										}elseif($result_key=='pend'){
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND `status`='Pending' AND delete_status !=1 ORDER BY entry_id DESC";
										}elseif($result_key=='comp'){
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND `status`='Completed' AND delete_status !=1 ORDER BY repair_date DESC";
										}elseif($result_key=='delv'){
											
											//PAGINATION CODE
											$sql_pagin = "SELECT COUNT(entry_id) FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND `status`='Delivered' AND delete_status !=1  AND rejected=0";
											$rs_sql_pagin = mysql_query($sql_pagin);
											$row_pagin = mysql_fetch_row($rs_sql_pagin);
											$total_records_pagin = $row_pagin[0];
											//echo $total_records_pagin;
											...
											//echo $page;exit;
											if($page >=2){
											    echo "<a href='view-cust-mobile-entry.php?result_key=delv&page=".($page-1)."'>  Prev </a>"; 
											}
											...
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND `status`='Delivered' AND delete_status !=1  AND rejected=0 ORDER BY delivered_date DESC LIMIT ".$start_from.", ".$offset."";
											
											
										}elseif($result_key=='rej'){
										
										//PAGINATION CODE
											$sql_pagin = "SELECT COUNT(entry_id) FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND `rejected`='1' AND delete_status !=1 AND `status` !='Delivered' ORDER BY `status` DESC ";
											...
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `store_id`=".$store_id." AND `rejected`='1' AND delete_status !=1 AND `status` !='Delivered' ORDER BY `entry_id` DESC LIMIT ".$start_from.", ".$offset."";
												...
												$defect_3='';
													...
													if($result_key=='pend'){
													  echo '<tr>
															...
															<td>'.$result_cust['imei_serial_num'].'</td>
															<td>'.$mob_def_result['defect_name'].$defect_2.$defect_3.'</td>
															<td>'.$result_cust['exp_delivery'].'</td>
															<td>'.$result_cust['cust_contact'].' / '.$result_cust['cust_alt_contact'].'</td>
										...
										if($result_cust['status']=='Completed'){
										?>
										...
										<?php
										}elseif($result_cust['status']=='Rejected'){
                    					?>
                    					<td><a href="./mobile-delivery-invoice.php?delivery-id=<?php echo $result_cust['entry_id']; ?>&action=inst_rej_del"><img style="border:none; width:65px; height:40px;" src="./img/inst_rej.jpg"/></a></td>
                    					<?php
										}else{
										?>
										<td><a href="./mobile-delivery-invoice.php?delivery-id=<?php echo $result_cust['entry_id']; ?>"><img style="border:none; width:65px; height:40px;" src="./img/inst_del.png"/></a></td>
