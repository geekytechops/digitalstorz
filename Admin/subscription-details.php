...
//$sql_plan="SELECT * FROM `cust_kolors_users_list` WHERE `user_role`='admin'";
...
*/
$sql_referal="SELECT `referal_code`,`kyc_status` FROM stores WHERE `store_id`=".$result_plan['store_id'];
$query_referal=mysql_query($sql_referal);
$result_referal=mysql_fetch_array($query_referal);
                    if($result_plan['mobile_no_verified'] == 'yes'){
                        $mobile_status="<span style='color:green;font-weight:bold;' >Verified</span>";
                    }else{
                        $mobile_status="<span style='color:red;font-weight:bold;' >Not Verified</span>";
                    }
                    echo '<tr>
