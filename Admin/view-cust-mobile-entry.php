	...
	<div class="main_container">
		...
		<div class="right_container_full">
			...
			<div class="content_holder">
				<div class="content_holder_heading">&#8803;&nbsp;&nbsp;View / Update Customer Jobs</div>
				<div class="content_holder_body">
				
				<div> 
					<form name="mobile_search"   autocomplete="off" method="post" action="./view-cust-mobile-entry.php" onsubmit="return validate()">
					<!--Job Search with Mobile: <input type="number" maxlength="10" name="mobile_search" value="" onkeypress="if(this.value.length==10) return false;"  onkeydown="javascript: return ['Backspace','Delete','ArrowLeft','ArrowRight'].includes(event.code) ? true : !isNaN(Number(event.key)) && event.code!=='Space'" >&nbsp;&nbsp;&nbsp;-->
					<!--Job Search with Mobile: <input type="number" maxlength="10" name="mobile_search" value="" onkeypress="if(this.value.length==10) return false;"  onkeydown="javascript: return ['Backspace','Delete','ArrowLeft','ArrowRight'].includes(event.code) ? true : !isNaN(Number(event.key)) && event.code!=='Space'" >&nbsp;&nbsp;&nbsp;-->
					Job Search with Mobile: <input type="number" maxlength="10" name="mobile_search" value="" ondrop="return false;" autocomplete="off" required required pattern="[0-9]{10}" onkeypress="if(this.value.length==10) return false;"  onkeydown="javascript: return (event.keyCode == 69 || event.keyCode == 190 || event.keyCode == 189 || event.keyCode == 187) ? false : true" >
				...
				<table class="order_hisory_table">
										...
										//PAGINATION CODE
											...
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `rejected`='1'  AND store_id=".$store_id." AND delete_status !=1 AND `status` !='Delivered' ORDER BY `repair_date` DESC LIMIT ".$start_from.", ".$offset."";
										...
										//PAGINATION CODE
											...
											$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `rejected`='1' AND delete_status !=1  AND store_id=".$store_id." AND `status` ='Delivered' ORDER BY delivered_date DESC LIMIT ".$start_from.", ".$offset."";
											//$sql_cust="SELECT * FROM `adm_cust_mob_add` WHERE `rejected`='1' AND delete_status !=1 ORDER BY entry_id DESC LIMIT ".$start_from.", ".$offset."";
											//echo $sql_cust;
										}
										//echo $sql_cust;
												...
												$result_getname_delby=mysql_fetch_array($query_get_name3);
													...
													if($result_key=='pend'){
													  echo '<tr>
															...
															<td><b>'.$del_msg_stat.$result_cust['status'].'</b></td>
															<td><a href="./print-job.php?job_id='.$result_cust['entry_id'].'" target="_blank"><img style="border:none; width:35px; height:35px;" src="./img/print_button.PNG"></a></td>
															'; //echo ends
													}elseif($result_key=='comp'){
													  echo '<tr>
															<td><b>'.$result_cust['entry_id'].'</b></td>
