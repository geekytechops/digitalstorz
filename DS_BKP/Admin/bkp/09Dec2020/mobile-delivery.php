...
if(isset($_REQUEST['submit'])){ //When form Submits
		...
		$sql_get_details="SELECT * FROM `adm_cust_mob_add` WHERE `cust_contact`=".$cust_mob_no." AND delete_status!=1 AND status='Completed'";
				...
				<form name="delivery" method="post" action="./mobile-delivery.php" onsubmit="return validate()">
					...
					<?php
						...
						$mob_def_result=mysql_fetch_array($mob_def_query);
							...
							echo '<tr >
										<td style="color:green;">FOR THIS CUSTOMER, NO MOBILES ARE READY FOR DELIVERY.. Please check Oders Tab.</td>
